...
def load(checkpoint_path, filename=_DEFAULT_HF_FILENAME, device="cuda"):
    ...
    SceneCutGUI: `device` added (upstream is cuda-only).
    """
    ...
    model, model_args = load_model(checkpoint_path, device=device)
